...
def microwave1(r,t):
    ...
    E_0 = np.sqrt(power/(eps0*area*c))
    E = math.sqrt(2)*E_0 * np.cos(2*math.pi/lamda*(r[:,2:3]-c*t))
    #print(E)
    freq = c/lamda
    if t%0.2==0: 
        print('freq',freq)
...
while t<0.01:
    #print(t)
    micro.append(circular_microwave(t))
    time_track.append(t)
    t += 1E-4
print(micro)
plt.plot(time_track,np.array(micro)[:,0])
plt.show()
'''
