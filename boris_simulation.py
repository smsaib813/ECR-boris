...
def initial_conditions(N=2): #number of particles
    ...
    #for 1 particle only:
    r_array_old = np.array([[1,0,0]])*8.37E-6
    print('r_array_old',r_array_old)
    r_array_new = np.zeros(shape = (N,3))
    #calculating B field
    B_array = B_ext
    print('B_array = ',B_array)
    return [q_array, mass_array, E_ext, B_array, r_array_old, r_array_new, v_array_old, v_array_new] 

def energy_investigation(injection_time = 0.1E-3,lamda=0.0015501773526777228, plot=True):
    [q_array, mass_array, E_ext, B_array, r_array_old, r_array_new, v_array_old, v_array_new]=initial_conditions()
    [v_track, r_track, t_track, Time_max, E_track, totalKE, trapU, intU]=calc_rvE(r_array_old, v_array_old,B_array,E_ext,lamda,injection_time)
    if plot:
        intU=0
...
def calc_rvE(r_array_old,v_array_old,B_array,E_ext,lamda,injection_time = 0.1E-3):
...
def plot_KE(t_track, Time_max, totalKE, trapU, intU,l=250000, injection_time = 0.1E-3): #readme needs modifying
    l =  np.size(t_track)
    start_op_time = time.time()
    ...
    plt.axvline(x = Time_max/4, color = 'r')#, label = 'start of microwave heating')
    plt.axvline(x = Time_max - injection_time, color = 'r')#, label = 'end of microwave heating')
    plt.ylabel('energy')
    plt.xlabel('time')
...
def plot_totE(t_track, Time_max, totalKE, trapU, intU, l=500000, injection_time=0.1E-3): 
    l = np.size(t_track)
    start_op_time = time.time()
    ...
    ax_energy1.axvline(x = Time_max/4, color = 'r')#, label = 'start of microwave heating')
    ax_energy1.axvline(x = 3*Time_max/4, color = 'r')#, label = 'end of microwave heating')
    plt.xlabel('t')
    plt.ylabel('energy')
...
#plotting potentials
def plot_potential(t_track, Time_max, trapU, intU): #readme needs modifying
    start_op_time = time.time()
    ...
    if np.size(trapU) == np.size(t_track):
        ...
        plt.axvline(x = Time_max/4, color = 'r')#, label = 'start of microwave heating')
        plt.axvline(x = 3*Time_max/4, color = 'r')#, label = 'end of microwave heating')
        plt.xlabel('t')
        plt.ylabel('V')
